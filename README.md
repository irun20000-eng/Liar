...
- **가족 찜 투표 + 실시간 공유**: 개요 탭에서 "가족방 만들기" → 링크를 가족에게 보내면 각자 폰에서 "나는 누구"를 고르고 찜한 내용이 5초 주기로 서로에게 반영(공개 JSON 저장소 jsonblob 기본, Firebase Realtime Database 주소 직접 연결 가능, 기기별 로컬 저장은 항상 유지)
