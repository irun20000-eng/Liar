...
| 라이어 역전 기회 | 지목된 라이어가 **후보 10개 중에서 골라** 진짜 제시어를 맞히면 역전승. |
...
## ❄️ 서울 겨울 가족여행 플래너 (`trip/`)

라이어 게임과는 별개의 정적 웹앱입니다. 같은 저장소의 `trip/` 폴더에 있으며, 배포 시 `…/Liar/trip/` 경로로 열립니다.

- **대상**: 2027.1.19(화) 저녁 ~ 1.23(토), 대전 출발 자차, 5인 가족(중2·초6·초3·엄마·아빠)
- **장소 풀**: 공연·체험·박물관·명소·맛집·근교 원정(강화·인천·파주·과천·수원·용인) 94곳 + 5인 1실 가능 숙소 10곳(비교표) — 운영시간·요금·예약·주차·비고·지도 링크, 초성 검색과 필터
- **추천 일정 6개 안**: 정석 / 실내 집중 / 강화도 원정 / 인천 원정 / 역사·과학 심화 / 스포츠·K-컬처+귀가길 수원 — 일자별 타임테이블(이동·비고 포함)
- **내 일정 만들기**: 풀에서 담아 순서 조정 → 이동시간·종료시각 자동 계산, 휴무·운영시간 충돌 경고, JSON 내보내기/가져오기, 인쇄
- **예산 계산기**: 숙박(선택 숙소 × 박수)·입장·맛집·미배정 끼니·간식·연료(주행거리 자동)·톨·주차·예비비를 절약/기본/여유 3안으로, 일자별 내역과 1인당 금액
- **가족 찜 투표 + 실시간 공유(가족방)**: 개요 탭 안내대로 Firebase Realtime Database(무료 Spark 요금제)를 만들고 주소를 붙여넣으면 가족방이 생성됩니다. 링크를 연 폰이 자동 합류하고, 각자 "나는 누구"를 고른 뒤 찜한 내용이 Firebase 스트리밍(SSE)으로 1~2초 안에 서로에게 반영됩니다(스트림 실패 시 5초 폴링). 셀 단위 타임스탬프 병합으로 동시 수정도 수렴하며, 기기별 로컬 저장은 항상 유지됩니다.
- **예매 D-day 체크리스트**, **Leaflet 지도 동선**, **공유 링크**(일정 전체를 URL 하나에), **PWA 오프라인**
- 순수 HTML/CSS/JS, 서버 없음, 데이터는 브라우저 localStorage에만 저장
...
> 정보 조사 기준 2026년 9월. "확인 필요" 표시 항목은 2027년 1월 운영·요금·라인업 확정 전이므로 공식 링크로 재확인하세요.
